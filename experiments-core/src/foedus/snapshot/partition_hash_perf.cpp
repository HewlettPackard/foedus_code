 ...
 */
/**
 * @file foedus/snapshot/partition_hash_perf.cpp
 * @brief Measures the performance of hash partitioner (both sorting and partitioning)
 * @author kimurhid
 * @details
 * So far 9.3M (2015/05/15 on Z820).
 */
...
const uint8_t  kBinBits = 21;  // 8 records per bin
...
void make_dummy_partitions(
  ...
  metadata->allocate_data(
    engine,
    &scope,
    sizeof(storage::hash::HashPartitionerData));
  storage::hash::HashStorage target(engine, id);
  storage::hash::HashPartitionerData* data
    = reinterpret_cast<storage::hash::HashPartitionerData*>(metadata->locate_data(engine));
  data->partitionable_ = true;
  data->bin_bits_ = target.get_bin_bits();
  data->bin_shifts_ = target.get_bin_shifts();
  data->levels_ = target.get_levels();
  data->total_bin_count_ = target.get_bin_count();
  for (uint64_t i = 0; i < data->total_bin_count_; ++i) {
    data->bin_owners_[i] = i % 16;
  }
...
void populate_logs(storage::StorageId id, char* buffer, BufferPosition* positions, uint64_t* size) {
  ...
  for (uint64_t i = 0; i < kRecords; ++i) {
    positions[i] = cur / 8;
    storage::hash::HashOverwriteLogType* log
...
int main_impl(int argc, char **argv) {
  gflags::SetUsageMessage("partition_hash_perf");
  ...
  options.storage_.partitioner_data_memory_mb_ = 30;  // because hash partitioner is mem hogging
  ...
  std::vector<std::string> papi_results;
  {
    ...
    COERCE_ERROR(engine.initialize());
    {
      ...
      Epoch commit_epoch;
      storage::hash::HashMetadata meta("aaa", kBinBits);
      storage::hash::HashStorage target;
      COERCE_ERROR(engine.get_storage_manager()->create_hash(&meta, &target, &commit_epoch));
