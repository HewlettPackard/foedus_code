...
ErrorStack SsspClientTask::run_impl_navigational() {
  ...
  while (!is_stop_requested()) {
    ...
    const uint64_t kNodePerBy = kNodesPerBlock * kPartitionSize;
    if ((in_partition_node % kNodePerBy) >= kNodePerBy - 64) {
      continue;
    }

    WRAP_ERROR_CODE(do_navigation(source_id, dest_id, &minheap));
...
ErrorCode SsspClientTask::do_navigation(
  ...
  NodeId dest_id,
  DijkstraMinheap* minheap) {
  hashtable_.clean();  // reuse the hashtable everytime. just clean it
  ...
  while (true) {
    ...
    while (true) {  // because we might have lazy-delete retry
      ...
      const DijkstraHashtable::Record* record = hashtable_.get(top.node_id_);
