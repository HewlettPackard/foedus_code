...
DijkstraHashtable::DijkstraHashtable()
  : memory_(),
    buckets_(nullptr),
    used_buckets_(nullptr),
    used_buckets_count_(0),
...
ErrorCode DijkstraHashtable::create_memory(uint16_t numa_node) {
  ...
  uint64_t used_bucket_memory_size = sizeof(uint16_t) * kMaxRecords;
  uint64_t next_memory_size = sizeof(Record) * kMaxRecords;
  uint64_t total_memory_size = bucket_memory_size + used_bucket_memory_size + next_memory_size;
  memory_.alloc(
    total_memory_size,
    1ULL << 21,
  ...
  used_buckets_ = reinterpret_cast<uint16_t*>(mem + pos);
  pos += used_bucket_memory_size;
...
void DijkstraHashtable::clean() {
  for (uint32_t i = 0; i < used_buckets_count_; ++i) {
    uint16_t b = used_buckets_[i];
    buckets_[b].init(0);
  }
  used_buckets_count_ = 0;
  next_records_count_ = 0;
...
std::ostream& operator<<(std::ostream& o, const DijkstraHashtable& v) {
  ...
  o << "  <used_buckets_count_>" << v.used_buckets_count_ << "</used_buckets_count_>" << std::endl;
