...
class RunTpcbTask {
 public:
  ...
  ErrorCode try_transaction(
    ...
    balance = amount;
    CHECK_ERROR_CODE(tellers_.increment_record_normalized(
      context,
      nm(teller_id),
      &balance,
      sizeof(uint64_t)));
    ...
    balance = amount;
    CHECK_ERROR_CODE(accounts_.increment_record_normalized(
      context,
      nm(account_id),
      &balance,
      sizeof(uint64_t)));
 ...
 private:
  ...
  MasstreeStorage branches_;
  MasstreeStorage tellers_;
  MasstreeStorage accounts_;
