...
do
  ...
  do
    ...
    echo "./tpcc -warehouses=$warehouses -take_snapshot=true -nvm_folder=/testnvm -fork_workers=$fork_workers -high_priority=$high_priority -null_log_device=false -loggers_per_node=$loggers_per_node -thread_per_node=$thread_per_node -numa_nodes=$numa_nodes -log_buffer_mb=$log_buffer_mb -neworder_remote_percent=1 -payment_remote_percent=15 -volatile_pool_size=$volatile_pool_size -snapshot_pool_size=$snapshot_pool_size -reducer_buffer_size=16 -duration_micro=$duration_micro"
    ./tpcc -warehouses=$warehouses -take_snapshot=true -nvm_folder=/testnvm -fork_workers=$fork_workers -high_priority=$high_priority -null_log_device=false -loggers_per_node=$loggers_per_node -thread_per_node=$thread_per_node -numa_nodes=$numa_nodes -log_buffer_mb=$log_buffer_mb -neworder_remote_percent=1 -payment_remote_percent=15 -volatile_pool_size=$volatile_pool_size -snapshot_pool_size=$snapshot_pool_size -reducer_buffer_size=16 -duration_micro=$duration_micro &> "result_tpcc_nvm_$machine_shortname.n$nvm_latency.r$rep.log"
    sudo umount /testnvm
  done
done
