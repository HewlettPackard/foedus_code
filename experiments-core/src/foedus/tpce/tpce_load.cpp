...
ErrorStack TpceLoadTask::load_warehouses() {
...
ErrorStack TpceLoadTask::load_districts() {
...
ErrorStack TpceLoadTask::load_items() {
...
ErrorStack TpceLoadTask::load_stocks() {
  Epoch ep;
  WRAP_ERROR_CODE(xct_manager_->begin_xct(context_, xct::kSerializable));
  StockData data;
  for (Wid wid = from_wid_; wid < to_wid_; ++wid) {
...
ErrorStack TpceLoadTask::load_customers() {
  for (Wid wid = from_wid_; wid < to_wid_; ++wid) {
    for (Did did = 0; did < kDistricts; ++did) {
...
#5  0x000000000047cbee in foedus::tpce::TpceLoadTask::load_customers_in_district(unsigned short, unsigned char) ()
#6  0x000000000047d5a0 in foedus::tpce::TpceLoadTask::load_customers() ()
#7  0x000000000047d9bb in foedus::tpce::TpceLoadTask::load_tables() ()
#8  0x000000000047e3ad in foedus::tpce::TpceLoadTask::run(foedus::thread::Thread*) ()
#9  0x000000000047e81f in foedus::tpce::tpce_load_task(foedus::proc::ProcArguments const&) ()
...
ErrorStack TpceLoadTask::load_customers_in_district(Wid wid, Did did) {
...
ErrorStack TpceLoadTask::load_orders() {
  for (Wid wid = from_wid_; wid < to_wid_; ++wid) {
    for (Did did = 0; did < kDistricts; ++did) {
...
ErrorStack TpceLoadTask::load_orders_in_district(Wid wid, Did did) {
...
void TpceLoadTask::random_orig(bool *orig) {
...
int32_t TpceLoadTask::make_alpha_string(int32_t min, int32_t max, char *str) {
...
int32_t TpceLoadTask::make_number_string(int32_t min, int32_t max, char *str) {
  const char *character =
    /***  "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"; */
    "1234567890";
  int32_t length = rnd_.uniform_within(min, max);
...
void TpceLoadTask::make_address(char *str1, char *str2, char *city, char *state, char *zip) {
...
Cid TpceLoadTask::get_permutation(bool* cid_array) {
  while (true) {
