...
class McsAdaptorInterface {
 public:
  ...
  McsRwBlock* get_rw_my_block(McsBlockIndex index);
  ...
  McsRwBlock* get_rw_other_block(thread::ThreadId id, McsBlockIndex index);

 private:
  McsAdaptorInterface() = delete;
  ~McsAdaptorInterface() = delete;
};

/**
 * A dummy implementation that provides McsAdaptorInterface for testing.
...
struct McsMockThread {
  ...
  std::vector<McsRwBlock> mcs_rw_blocks_;
...
struct McsMockNode {
  ...
  std::vector<McsMockThread>  threads_;
};

struct McsMockContext {
  McsMockContext(uint32_t nodes, uint32_t threads_per_node, uint32_t max_block_count) {
    nodes_.resize(nodes);
  ...
  std::vector<McsMockNode>    nodes_;
};

class McsMockAdaptor {
 public:
  McsMockAdaptor(thread::ThreadId id, McsMockContext* context)
    : id_(id),
  ...
  McsBlock* get_ww_my_block(McsBlockIndex index) { return me_->mcs_ww_blocks_.data() + index; }
  McsRwBlock* get_rw_my_block(McsBlockIndex index) { return me_->mcs_rw_blocks_.data() + index; }

  McsMockThread* get_other_thread(thread::ThreadId id) {
  ...
  std::atomic<bool>* other_waiting(thread::ThreadId id) {
    McsMockThread* other = get_other_thread(id);
 ...
 private:
  ...
  McsMockContext* const       context_;
  McsMockThread* const        me_;
};
