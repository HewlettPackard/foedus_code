...
#define X(a, b, c) case a: \
  reinterpret_cast< c* >(buffer)->apply_storage(context, storage_id); return;
void invoke_apply_storage(void *buffer, thread::Thread* context, storage::StorageId storage_id) {
  invoke_assert_valid(buffer);
  LogHeader* header = reinterpret_cast<LogHeader*>(buffer);
  LogCode code = header->get_type();
  switch (code) {
