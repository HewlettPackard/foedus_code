...
ArrayStorage::ArrayStorage(Engine* engine, StorageId id, const std::string &name,
    ...
    pimpl_ = new ArrayStoragePimpl(engine, id, name, payload_size, array_size, root_page, create);
}
...
ErrorStack  ArrayStorage::overwrite_record(ArrayOffset offset, const void *payload) {
    return pimpl_->overwrite_record(offset, payload);
}
ErrorStack  ArrayStorage::overwrite_record_part(ArrayOffset offset, const void *payload,
                            uint16_t payload_offset, uint16_t payload_count) {
    return pimpl_->overwrite_record_part(offset, payload, payload_offset, payload_count);
}
