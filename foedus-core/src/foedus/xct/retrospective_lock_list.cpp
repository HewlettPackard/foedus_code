...
////////////////////////////////////////////////////////////
LockListPosition CurrentLockList::get_or_add_entry(RwLockableXctId* lock) {
  ...
  if (last_active_entry_ == kLockListPositionInvalid) {
    ...
    array_[new_pos].set(id, lock, kNoLock);
    return new_pos;
  }
  ...
  array_[insert_pos].set(id, lock, kNoLock);
  return insert_pos;
}
...
void CurrentLockList::batch_insert_write_placeholders(
  ...
  for (LockListPosition pos = 1U; pos <= last_active_entry_ && write_pos < write_set_size; ++pos) {
    CurrentLock* existing = array_ + pos;
    WriteXctAccess* write = write_set + write_pos;
    ...
    if (existing->universal_lock_id_ < write_lock_id) {
      ...
      ++added;
      CurrentLock* new_entry = array_ + last_active_entry_ + added;
      new_entry->set(write_lock_id, write->owner_id_address_, kNoLock);
      ...
      for (++write_pos; write_pos < write_set_size; ++write_pos) {
        WriteXctAccess* next_write = write_set + write_pos;
