...
bool has_enough_max_map_count() {
  uint64_t map_count = EngineOptions::read_int_from_proc_fs(
    "/proc/sys/vm/max_map_count",
    &std::cout);
  return (map_count > 65530U);
}
...
EngineOptions get_big_options() {
  ...
  } else if (!has_enough_max_map_count()) {
    std::cerr << "You haven't setup max_map_count!" << std::endl;
    options.memory_.rigorous_page_boundary_check_ = false;
  }
...
EngineOptions get_tiny_options() {
  ...
  if (RUNNING_ON_VALGRIND) {  // see rigorous_page_boundary_check_'s doc on why
    ...
    // Although it's due to mis-configuration, it's less confusing to automatically
    // turn it off if the user hasn't setup this environment value.
    // eg: "wow. lots of testcases fail. bug?" ....(hours later) "oh, crap, max_map_count issue"
    // Instead, we have just one testcase that checks this misconfiguration. (check_env testcase)
    std::cerr << "You haven't setup max_map_count!" << std::endl;
    options.memory_.rigorous_page_boundary_check_ = false;
  }
  options.cache_.snapshot_cache_size_mb_per_node_ = 2;
